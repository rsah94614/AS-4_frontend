            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6 text-center sm:text-left">
                ...
                <div className="space-y-4 sm:pt-2">
                    ...
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-1">
                        <Badge variant={profile.is_active ? "default" : "destructive"} className={`gap-1.5 px-2.5 py-0.5 rounded-md ${profile.is_active ? 'bg-(--chart-5)/10 text-chart-5 hover:bg-(--chart-5)/20 border-(--chart-5)/20' : 'bg-destructive/10 text-destructive hover:bg-destructive/20 border-destructive/20'} border`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${profile.is_active ? "bg-chart-5" : "bg-destructive"}`} />
                            {profile.is_active ? "Active" : "Inactive"}
                        </Badge>

                        {profile.roles.map((r) => (
                            <Badge key={r.role_id} variant="secondary" className="bg-accent text-accent-foreground hover:bg-accent/80 border-border border px-2.5 py-0.5 rounded-md">
                                {r.role_name}
                            </Badge>
                        ))}
                    </div>
